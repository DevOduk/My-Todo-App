    ...
    <div className="Home">
      ...
      <nav className='container p-0 mt-3 mb-2 px-3 d-flex justify-content-between align-items-center'>
        ...
        <Avatar className='border' src={storedAvatar} onClick={()=> setShowProfile(true)}/>
          {
            showProfile && (
              <motion.div ref={profileRef} className='border rounded-3 p-2 py-3 menuBox shadow-lg' style={{width: '350px', position: 'absolute', right: 0, top: 0}}
              variants={{
                hidden: { opacity: 0, x: 160, y: -85, scale: 0 }, // Start from top-right
                show: { 
                  opacity: 1, 
                  x: 0, 
                  y: 0, 
